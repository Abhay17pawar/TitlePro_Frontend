  ...
  const handleFormSubmit = async (data) => {
    try {
      ...
      if (result.success) {
        ...
        toast.error(result.message || "error while adding State", { autoClose: 1500 });
      }
    } catch (error) {
      const errorMessage = error.response?.data?.error?.errorMessage || "An error occurred while adding State.";
